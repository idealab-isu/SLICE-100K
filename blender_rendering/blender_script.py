...
def render_object(
    ...
    # first 4 are isometric, next 6 are orthographic
    isometric_poses = ['iso1', 'iso2', 'iso3', 'iso4']
    orthographic_poses = ['Top', 'Down', 'Left', 'Right', 'Front', 'Back']
    total_poses = len(isometric_poses) + len(orthographic_poses) # total poses = 10
        
    # render the images
    for i in range(total_poses):
        if i < len(isometric_poses):
            camera = set_camera_principle_axes(isometric_poses[i])
            save_name = f"iso_{i:03d}.png"
            save_npy = f"iso_{i:03d}.npy"
        else:
            camera = set_camera_principle_axes(orthographic_poses[i - len(isometric_poses)])
            save_name = f"ortho_{i-len(isometric_poses):03d}.png"
            save_npy = f"ortho_{i-len(isometric_poses):03d}.npy"

        render_path = os.path.join(output_dir, save_name)
        scene.render.filepath = render_path
        bpy.ops.render.render(write_still=True)

        rt_matrix = get_3x4_RT_matrix_from_blender(camera)
        rt_matrix_path = os.path.join(output_dir, save_npy)
        np.save(rt_matrix_path, rt_matrix)
    
    ######### ONLY FOR DEBUGGING PURPOSES #########
    # Save the blend file
    # blend_filepath = os.path.join(output_dir, "scene.blend")
