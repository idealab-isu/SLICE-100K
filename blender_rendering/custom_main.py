...
def handle_found_object(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if platform.system() == "Linux" and using_gpu:
            ...
            # As far as I know, MacOS does not support BLENDER_EEVEE, which uses GPU
        ...
        command = f"ADD YOUR PATH HERE -b --python blender_script.py -- {args}"
...
def get_example_objects() -> pd.DataFrame:
    ...
    return pd.read_json("ADD_PATH_YOUR_PATH/file_list.json", orient="records")
...
def render_objects(
    ...
    total_renders = 10
    data_dir = "ADD_YOUR_PATH_TO_STL_FILES_HERE"
    save_dir = "ADD_YOUR_PATH_TO_SAVE_RENDERS_HERE"
    for idx, object_row in tqdm(objects.iterrows()):
        local_path = os.path.join(data_dir, object_row['fileIdentifier'])
        
        if not os.path.exists(local_path):
            logger.error(f"File {local_path} does not exist.")
            continue

        handle_found_object(local_path=local_path,
                            file_identifier=object_row['fileIdentifier'],
