...
def remove_floats(text):
    """
    Removes all floats from the text.

    Args:
        text: The text to remove floats from.

    Returns:
        The text with all floats removed.
    """
    text_array = list(text)
    for i in range(1,len(text_array)-1):
        char = text_array[i]
        if char=="." and text_array[i-1].isdigit() and text_array[i+1].isdigit():
            start=i-1
            end=i+1
            text_array[i]=""
            while start-1>=0 and text_array[start-1].isdigit():
                start-=1
                text_array[start]=""
            while end+1<len(text_array) and text_array[end+1].isdigit():
                end+=1
                text_array[end]=""
...
def make_json(n_files,chunk_size):
    ...
    # for i,file_name_a in enumerate(os.listdir(gcode_path_a)[25:26]):
        ...
        # debug_output = '\n'.join(content_b.split('\n')[:500])
        # write flipped content into a g-code file
        # with open('sailfish_flipped.gcode', 'w') as f:
        #     f.write(file_b.split('G28')[0] + debug_output)
        # try:
        #     chunk_list += aligned_chunks(content_a,content_b,chunk_size)
        # except:
        #     print('FAILURE!')
    out_list = []
...
def fixed_length_chunking(text_a,text_b,chunk_size=20):
    text_a_lines = text_a.strip().split('\n')
    text_b_lines = text_b.strip().split('\n')
    num_lines = len(text_a_lines)
    start_i = 0
    end_i = min(start_i+chunk_size,num_lines)
    start_j = 0
    chunks = []
    while start_i<num_lines:
        chunk_a = "".join( x+"\n" for x in text_a_lines[start_i:end_i])
        chunk_b = "".join( x+"\n" for x in text_b_lines[start_j:end_i])
        chunks.append({"text_1":chunk_a,"text_2":chunk_b})
        start_i = end_i
        start_j = end_i
        end_i = min(start_i+chunk_size,num_lines)
...
def insert_delimiters_G(content):
